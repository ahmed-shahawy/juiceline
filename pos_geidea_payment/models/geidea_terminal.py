...
class GeideaTerminal(models.Model):
    ...
    def _compute_transaction_count(self):
        for terminal in self:
            terminal.transaction_count = len(terminal.transaction_ids)
    
    @api.depends('transaction_ids.state')
    def _compute_success_rate(self):
        for terminal in self:
            total = len(terminal.transaction_ids)
            if total > 0:
                successful = len(terminal.transaction_ids.filtered(lambda t: t.state == 'completed'))
                terminal.success_rate = (successful / total) * 100
            else:
                terminal.success_rate = 0.0
